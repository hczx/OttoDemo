...
public class MyGalleray extends Gallery implements
     ...
     * ImageView组
     */
    List<SimpleDraweeView> listImgs;
    ...
    public interface MyGalleryData {
     ...
     * @param galleryData
     */
    public void start(MyGalleryData galleryData) {

        this.mContext = galleryData.setContext();
        this.mUris = galleryData.setMris();
        this.mAdsId = galleryData.setAdsId();
        this.mSwitchTime = galleryData.setSwitchTime();
        this.mOvalLayout = galleryData.setOvaLayout();
        this.mFocusedId = galleryData.setFocusedId();
        this.mNormalId = galleryData.setNormalId();
        ininImages();// 初始化图片组
    ...
    private void ininImages() {
        listImgs = new ArrayList<SimpleDraweeView>(); // 图片组
        int len = mUris != null ? mUris.length : mAdsId.length;
        for (int i = 0; i < len; i++) {
            if (mLayoutInflater == null) {
                mLayoutInflater = LayoutInflater.from(mContext);
            }
            SimpleDraweeView simpleDraweeView = (SimpleDraweeView) mLayoutInflater.inflate(R.layout.simple_fresco_image, null);
//            ImageView imageview = new ImageView(mContext); // 实例化ImageView的对象
//            imageview.setScaleType(ImageView.ScaleType.FIT_XY); // 设置缩放方式
            simpleDraweeView.setLayoutParams(new LayoutParams(
                    LayoutParams.MATCH_PARENT,
                    LayoutParams.MATCH_PARENT));
            if (mUris == null) {// 本地加载图片
//                imageview.setImageResource(mAdsId[i]); // 为ImageView设置要显示的图片
                simpleDraweeView.setImageResource(mAdsId[i]);
            } else { // 网络加载图片
//                Log.i("jam", "ttttttttttttttttttttttttt----" + HttpOperataion.URL_TITLE + mUris[i]);
//                LoadingImgUtil.loadimgAnimateOption(mUris[i], imageview);
                simpleDraweeView.setImageURI(Uri.parse(mUris[i]));
            }
            listImgs.add(simpleDraweeView);
        }
...
//            imageview.setScaleType(ImageView.ScaleType.CENTER_INSIDE); // 设置缩放方式
            simpleDraweeView.setLayoutParams(new LayoutParams(
                    ...
                    LayoutParams.MATCH_PARENT));
            if (mUris == null) {// 本地加载图片
                simpleDraweeView.setImageResource(mAdsId[i]); // 为ImageView设置要显示的图片
            } else { // 网络加载图片
//                LoadingImgUtil.loadimgAnimateOption(mUris[i], imageview);
                simpleDraweeView.setImageURI(Uri.parse(mUris[i]));
            }
