...
public class AccountManager implements AccountProvider {
    ...
    private void loadAccountDataFromPrefs() {
        ...
        if (!TextUtils.isEmpty(accountJson)) {
            mCurrentAccount = (Account) BaseApplication.getInstance().accountFromJson(accountJson);
        }
    }
    ...
    private static Prefs prefs() {
        return Prefs.with(BaseApplication.appContext(), PREFS_ACCOUNT_STORAGE, Context.MODE_PRIVATE);
    }
